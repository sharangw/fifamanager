...
allows this extension to be used for all ``executemany()`` static calls
...
The default parameter static for the psycopg2 dialect is "pyformat", where
SQL is rendered using ``%(paramname)s`` static.   This format has the limitation
...
Where above, the dictionary static is converted into a tuple with positional
static.
